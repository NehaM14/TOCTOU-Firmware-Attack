...
void* copy_firmware(void* ptr)
{
	...
	// Do not allow copying if the active variable is true
	if  (strcmp(conn->shm_active,"true") == 0){
		printf("Cannot modify Isolated Memory, since ACTIVE is true\n\n");
		write(conn->driver_socket, (char *)"FAILED", strlen("FAILED"));
		return -1;
	}
	
	// Copy the firmware to the isolated memory of the device
	read(conn->driver_socket, &isolated_memory, sizeof(isolated_memory));
	strcpy(conn->shm_active,"true");
...
int main()
{
	//shared buffer for passing active
	int shmid;
    key_t key;
    char *shm;
